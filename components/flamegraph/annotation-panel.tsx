...
export function AnnotationPanel({
	...
	const isAnnotationVisible = (annotation: Annotation): boolean => {
		...
		if (annotation.type === 'highlight') {
			...
			const fromVisible = fromFrame
				? fromFrame.end >= startTime && fromFrame.start <= endTime
				: false;
			const toVisible = toFrame
				? toFrame.end >= startTime && toFrame.start <= endTime
				: false;
	...
	return (
		<div className={panelClassName}>
			<div className="flex min-w-max items-start space-x-6 p-3">
				{annotations.map((annotation) => {
					...
					if (annotation.type === 'highlight') {
						return (
							...
							>
								<div
									className="rounded-full px-3 py-1 text-xs font-medium whitespace-nowrap text-white"
									style={{
										backgroundColor: '#f43f5e',
									}}
								>
									{Math.round(annotation.startTime)} -{' '}
									{Math.round(annotation.endTime)} ms
								</div>
								<span className="text-perfagent-text mt-1 text-xs font-medium">
									{annotation.label}
								</span>
							</div>
						);
						...
						return (
							...
							>
								<div className="flex items-center space-x-1">
									<span className="rounded-full bg-gray-200 px-3 py-1 text-xs font-medium whitespace-nowrap text-gray-800">
										{fromName}
									</span>
					...
					if (annotation.type === 'label') {
						...
						return (
							<div
								...
								onClick={handleClick}
							>
								<div className="rounded-full bg-gray-200 px-3 py-1 text-xs font-medium whitespace-nowrap text-gray-800">
									{frameName}
								</div>
								<span className="text-perfagent-text mt-1 text-xs font-medium">
									{annotation.label}
