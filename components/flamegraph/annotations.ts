import type { Annotation, HighlightAnnotation, LinkAnnotation, LabelAnnotation, FrameNode, ViewState } from "./types"
...
export const sampleAnnotations: Annotation[] = [
  ...
  // Link annotation - linking uT at 50.40ms to i at 116ms
  {
    id: "link-1",
    type: "link",
    fromFrameId: "1-1-uT-50.4", // This will be matched to the actual frame ID in the renderer
    toFrameId: "1-1-i-116", // This will be matched to the actual frame ID in the renderer
  ...
  // Label annotation - labeling uD at 71.70ms as 'Forced Layout'
  {
    id: "label-1",
    type: "label",
    frameId: "1-1-uD-71.7", // This will be matched to the actual frame ID in the renderer
    label: "Forced Layout",
...
export function renderAnnotations(
  ...
  annotations.forEach((annotation) => {
    if (annotation.type === "highlight") {
      renderHighlightAnnotation(ctx, annotation, viewState, dimensions, selectedAnnotationId, yOffset)
    } else if (annotation.type === "link") {
      renderLinkAnnotation(ctx, annotation, viewState, dimensions, frameMap, selectedAnnotationId, yOffset)
    } else if (annotation.type === "label") {
      renderLabelAnnotation(ctx, annotation, viewState, dimensions, frameMap, selectedAnnotationId, yOffset)
    }
  })
...
function renderHighlightAnnotation(
  ...
  const annotationStartX = Math.max(0, ((annotation.startTime - startTime) / timeRange) * width)
  const annotationEndX = Math.min(width, ((annotation.endTime - startTime) / timeRange) * width)
  ...
  ctx.strokeStyle = darkenColor(annotation.color || "#e6d7c3", isSelected ? 0.45 : 0.3)
  ...
  if (annotationWidth > 80) {
    ...
    ctx.fillText(`${annotation.startTime.toFixed(1)}ms - ${annotation.endTime.toFixed(1)}ms`, labelX, labelY - 16)
  }
}
...
function renderLinkAnnotation(
  ...
  const fromDepthVisible = fromFrame.depth >= topDepth && fromFrame.depth < topDepth + visibleDepthCount
  const toDepthVisible = toFrame.depth >= topDepth && toFrame.depth < topDepth + visibleDepthCount
  ...
  if (fromFrame.end <= startTime) {
    ...
    fromStartX = Math.max(0, ((Math.max(fromFrame.start, startTime) - startTime) / timeRange) * width)
    fromEndX = Math.min(width, ((Math.min(fromFrame.end, endTime) - startTime) / timeRange) * width)
  }
  ...
  if (toFrame.end <= startTime) {
    ...
    // Frame is at least partially visible
    toStartX = Math.max(0, ((Math.max(toFrame.start, startTime) - startTime) / timeRange) * width)
    toEndX = Math.min(width, ((Math.min(toFrame.end, endTime) - startTime) / timeRange) * width)
  }
...
function renderLabelAnnotation(
  ...
  const frameX = ((frame.start + (frame.end - frame.start) / 2 - startTime) / timeRange) * width
  ...
  roundRect(ctx, frameX - labelWidth / 2, frameY - labelHeight - 10, labelWidth, labelHeight, 4)
  ctx.fill()
...
function findFrameById(frameMap: Map<string, FrameNode>, frameId: string): FrameNode | undefined {
...
function isFrameVisible(frame: FrameNode, viewState: ViewState): boolean {
  ...
  const depthVisible = frame.depth >= topDepth && frame.depth < topDepth + visibleDepthCount

  return timeVisible && depthVisible
}

// Find annotation at a specific time (for highlight annotations)
export function findAnnotationAt(time: number, annotations: Annotation[]): Annotation | null {
  ...
  return highlightAnnotations.find((annotation) => time >= annotation.startTime && time <= annotation.endTime) || null
}

// Find annotation by frame ID (for link and label annotations)
export function findAnnotationByFrameId(frameId: string, annotations: Annotation[]): Annotation | null {
  return (
    annotations.find(
      (annotation) =>
        (annotation.type === "label" && annotation.frameId === frameId) ||
        (annotation.type === "link" && (annotation.fromFrameId === frameId || annotation.toFrameId === frameId)),
    ) || null
  )
}
