...
export function useChat(): UseChat {
          ...
          for (const line of lines) {
            try {
              ...
              if (
                ...
                console.log("Suggestions found in toolResult:", parsedChunk.toolResult.suggestions)
                setSuggestions(parsedChunk.toolResult.suggestions || [])
                setSuggestionsLoading(false)
                suggestionsReceived = true
              } else if (parsedChunk.type === "data" && parsedChunk.data && parsedChunk.data.suggestions) {
                // Handle alternative format where suggestions might be in data.suggestions
                console.log("Alternative suggestions format found in data:", parsedChunk.data.suggestions)
                setSuggestions(parsedChunk.data.suggestions || [])
                setSuggestionsLoading(false)
                suggestionsReceived = true
              } else if (parsedChunk.suggestions) {
                // Handle direct suggestions array
                console.log("Direct suggestions array found:", parsedChunk.suggestions)
                ...
                if (parsedChunk.toolResult && parsedChunk.toolResult.suggestions) {
                  console.log("Suggestions found in generic toolResult:", parsedChunk.toolResult.suggestions)
                  setSuggestions(parsedChunk.toolResult.suggestions || [])
                  setSuggestionsLoading(false)
                  suggestionsReceived = true
                }
  ...
  const sendMessage = useCallback(
    async (content: string, files?: AttachedFile[]) => {
      ...
      try {
            ...
            try {
              ...
              if (parsedChunk.type === "text-delta") {
                ...
                setMessages((prev) => {
                  ...
                  const textPartIndex = updatedParts.findIndex((part) => part.type === "text")
                  ...
                  if (textPartIndex === -1) {
                    ...
                    updatedParts[textPartIndex] = {
                      ...updatedParts[textPartIndex],
                      text: (updatedParts[textPartIndex].text || "") + (parsedChunk.text || ""),
                    }
                  }
                  ...
                  setMessages((prev) => {
                    ...
                    return [
                      ...prev.slice(0, -1),
                      {
                        ...lastMessage,
                        toolCall: {
                          type: toolCall.toolName.replace(/^generate|^perform|^open/, "").toLowerCase(),
                ...
                setMessages((prev) => {
                  ...
                  const updatedAnnotations = [...(lastMessage.annotations || [])]
        ...
        if (error.name === "AbortError" || (error.message && error.message.includes("499"))) {
          ...
          setMessages((prev) => {
            ...
            if (lastMessage.role === "assistant" && (lastMessage.isWaiting || lastMessage.isStreaming)) {
              return [
                ...prev.slice(0, -1),
                {
                  id: lastMessage.id,
  ...
  const handleSubmit = useCallback(
    ...
    [input, attachedFiles, sendMessage, setInput, setAttachedFiles, setSuggestions, setSuggestionsLoading],
  )
  ...
  const stop = useCallback(
    (toolCallId?: string) => {
      ...
      if (typeof window !== "undefined" && !(window as any).cancelledToolCalls) {
      ...
      if (toolCallId) {
        ...
        const messageWithToolCall = messages.find((msg) => msg.toolCall && msg.toolCall.toolCallId === toolCallId)
  ...
  const handleFileChange = useCallback(
    ...
    [attachedFiles, setAttachedFiles, setSuggestionsLoading, triggerSuggestionsTool],
  )
